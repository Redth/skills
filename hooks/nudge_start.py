#!/usr/bin/env python3
"""
skill-reflect — Claude Code SessionStart nudge hook (Tier A).

Claude Code invokes this script at session start, passing hook context as
JSON on stdin (session_id, cwd, hook_event_name).
...
def _load_nudge_config(cwd: str | None) -> dict:
    """Return nudge section of config, merged with defaults. Never throws."""
...
def _collect_pending_skills(markers: list[Path], never_for: list[str]) -> list[str]:
    """Read marker files and collect distinct skill names, respecting neverForSkills."""
    seen: set[str] = set()
    for mp in markers:
        try:
...
# ─── REPO DETECTION (for neverForRepos) ──────────────────────────────────────
